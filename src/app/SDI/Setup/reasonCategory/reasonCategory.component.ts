...
declare let jsPDF;
@Component({
  selector: 'app-reasonCategory',
  templateUrl: './reasonCategory.component.html',
  styleUrls: ['./reasonCategory.component.css']
})
export class ReasonCategoryComponent implements OnInit {

  myForm: FormGroup;
  msgs: Message[] = [];
  errorMessage: string;
  successMessage: string;
  saveEdit: ReasonCatagory;
  allData: ReasonCatagory[];
  maxData: ReasonCatagory[];
  genaratedId: string;
  newGenaratedId: string;
  createdDate: Date = new Date();
  createdDate2 = new Date().toDateString();
  createdBy = 'Admin';
  selected: ReasonCatagory = new ReasonCatagory(0, '', '', null, '', '');
  mode = 'Observable';
  public events: any[] = [];

  constructor(private reasonCatagoryService: ReasonCatagoryService, private formBuilder: FormBuilder, public snackBar: MatSnackBar) { }
  convert() {
    var doc = new jsPDF();
    var col = ['Reason Category ID', 'Reason Category'];
    var rows = [];
    for (let j = 0; j < this.allData.length; j++) {

      var temp = [this.allData[j].genaratedId, this.allData[j].reasonCatagory];
      rows.push(temp);
    }
    ...
    doc.text(155, 20, 'All Reason Categories');
    ...
    this.reasonCatagoryService.getAllData()
      ...
      resObj => {
        ...
        if (webServResponce.statusId == 200) {
          this.allData = <ReasonCatagory[]>webServResponce.result;
        } else {
  ...
  getMax() {
    let webServResponce: WebServResponce;
    this.reasonCatagoryService.getMaxData()
      .subscribe(
      resObj => {
        webServResponce = resObj;
        if (webServResponce.statusId == 200) {
          this.maxData = <ReasonCatagory[]>webServResponce.result;
  ...
  addNew() {
    ...
    this.selected = new ReasonCatagory(0, '', '', null, '', '');
    this.myForm.reset();
  ...
  saveOrEdit(data: ReasonCatagory) {
    let webServResponce: WebServResponce;
    this.saveEdit = new ReasonCatagory(data.id, data.genaratedId, data.reasonCatagory, this.createdDate, this.createdBy, '');
    ...
    if (b) {
      if (data.id == 0) {
        ...
        this.reasonCatagoryService.saveData(this.saveEdit)
          .subscribe(
          resObj => {
            webServResponce = resObj;
        ...
        this.saveEdit = new ReasonCatagory(data.id, data.genaratedId, data.reasonCatagory, this.createdDate, this.createdBy, '');
        this.reasonCatagoryService.editData(this.saveEdit)
          .subscribe(
          resObj => {
            webServResponce = resObj;
            if (webServResponce.statusId == 200) {
  ...
  deleteById(id: number) {
    let webServResponce: WebServResponce;
    this.reasonCatagoryService.deleteData(id).subscribe(
      resObj => {
        webServResponce = resObj;
        if (webServResponce.statusId == 200) {
          this.getAll();
  ...
  isAllReadyExits(data: ReasonCatagory): boolean {
    let obj: ReasonCatagory = this.allData
      .filter(ReasonCatagory => ReasonCatagory.reasonCatagory === data.reasonCatagory)
      .pop();
  ...
  ngOnInit() {
    ...
    this.myForm = this.formBuilder.group({
      ...
      'reasonCatagory': ['', Validators.compose([Validators.required, Validators.pattern('^[a-zA-Z](?:[a-zA-Z ]*[a-zA-Z])?$'), Validators.maxLength(50)])]
    });
  }
