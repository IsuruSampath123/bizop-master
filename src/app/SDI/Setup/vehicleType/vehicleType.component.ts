...
export class VehicleTypeComponent implements OnInit {
  ...
  createdBy = 'Admin';
  selected: VehicleType = new VehicleType(0, '', '', null, '', '');
  mode = 'Observable';
  public events: any[] = [];

  constructor(private vehicleTypeService: VehicleTypeService,private formBuilder: FormBuilder,public snackBar: MatSnackBar) {
  ...
  convert() {
    var doc = new jsPDF();
    var col = ['Vehicle Type ID', 'Vehicle Type'];
    var rows = [];
    for (let j = 0; j < this.allData.length; j++) {

      var temp = [this.allData[j].genaratedId, this.allData[j].vehicleType];
      rows.push(temp);
    }
    ...
    doc.text(155, 20, 'All Vehicle Types');
  ...
  getAll() {
    ...
    this.vehicleTypeService.getAllData()
      .subscribe(
      resObj => {
        webServResponce = resObj;
        if (webServResponce.statusId == 200) {
          this.allData = <VehicleType[]>webServResponce.result;
        } else {
  ...
  getMax() {
    let webServResponce: WebServResponce;
    this.vehicleTypeService.getMaxData()
      .subscribe(
      resObj => {
        webServResponce = resObj;
        if (webServResponce.statusId == 200) {
          this.maxData = <VehicleType[]>webServResponce.result;
  ...
  addNew() {
    ...
    this.selected = new VehicleType(0, '', '', null, '', '');
  ...
  saveOrEdit(data: VehicleType) {
    let webServResponce: WebServResponce;
    this.saveEdit = new VehicleType(data.id, data.genaratedId, data.vehicleType, this.createdDate, this.createdBy, '');
    ...
    if (b) {
      if (data.id == 0) {
        this.selected = data;
        this.saveEdit = new VehicleType(data.id, data.genaratedId, data.vehicleType, this.createdDate, this.createdBy, '');
        this.vehicleTypeService.saveData(this.saveEdit)
          .subscribe(
          resObj => {
            webServResponce = resObj;
            if (webServResponce.statusId == 200) {
        ...
        this.saveEdit = new VehicleType(data.id, data.genaratedId, data.vehicleType, this.createdDate, this.createdBy, '');
        this.vehicleTypeService.editData(this.saveEdit)
          .subscribe(
          resObj => {
            webServResponce = resObj;
            if (webServResponce.statusId == 200) {
  ...
  deleteById(id: number) {
    let webServResponce: WebServResponce;
    this.vehicleTypeService.deleteData(id).subscribe(
      resObj => {
        webServResponce = resObj;
        if (webServResponce.statusId == 200) {
          this.getAll();
  ...
  isAllReadyExits(data: VehicleType): boolean {
    let obj: VehicleType = this.allData
      .filter(VehicleType => VehicleType.vehicleType.toLowerCase() === data.vehicleType.toLowerCase())
      .pop();
  ...
  ngOnInit() {
    //console.log(this.refeshService.getValue());
    //  this.refeshService.setValue(false);
    ...
    this.myForm = this.formBuilder.group({
      ...
      'vehicleType': ['',Validators.compose([Validators.required, Validators.pattern('^[a-zA-Z](?:[a-zA-Z ]*[a-zA-Z])?$'), Validators.maxLength(50)])]
    });
  }
