...
export class DepartmentComponent implements OnInit {
  ...
  createdBy = 'Admin';
  selected: Department = new Department(0, '', '', null, '', '');
  mode = 'Observable';
  public events: any[] = [];

  constructor(private departmentService: DepartmentService, private formBuilder: FormBuilder, public snackBar: MatSnackBar) { }


  convert() {
    var doc = new jsPDF();
    var col = ['Department ID', 'Department'];
    var rows = [];
    for (let j = 0; j < this.allData.length; j++) {

      var temp = [this.allData[j].genaratedId, this.allData[j].department];
      rows.push(temp);
    }
    ...
    doc.text(155, 20, 'All Deparments');
  ...
  getAll() {
    ...
    this.departmentService.getAllData()
      .subscribe(
      resObj => {
        webServResponce = resObj;
        if (webServResponce.statusId == 200) {
          this.allData = <Department[]>webServResponce.result;
        } else {
  ...
  getMax() {
    let webServResponce: WebServResponce;
    this.departmentService.getMaxData()
      .subscribe(
      resObj => {
        webServResponce = resObj;
        if (webServResponce.statusId == 200) {
          this.maxData = <Department[]>webServResponce.result;
  ...
  addNew() {
    ...
    this.selected = new Department(0, '', '', null, '', '');
    this.myForm.reset();
  ...
  saveOrEdit(data: Department) {
    let webServResponce: WebServResponce;
    this.saveEdit = new Department(data.id, data.genaratedId, data.department, this.createdDate, this.createdBy, '');
    ...
    if (b) {
      if (data.id == 0) {
        ...
        this.departmentService.saveData(this.saveEdit)
          .subscribe(
          resObj => {
            webServResponce = resObj;
        ...
        this.saveEdit = new Department(data.id, data.genaratedId, data.department, this.createdDate, this.createdBy, '');
        this.departmentService.editData(this.saveEdit)
          .subscribe(
          resObj => {
            webServResponce = resObj;
            if (webServResponce.statusId == 200) {
  ...
  deleteById(id: number) {
    let webServResponce: WebServResponce;
    this.departmentService.deleteData(id).subscribe(
      resObj => {
        webServResponce = resObj;
        if (webServResponce.statusId == 200) {
          this.getAll();
  ...
  isAllReadyExits(data: Department): boolean {
    let obj: Department = this.allData
      .filter(Department => Department.department.toLowerCase() === data.department.toLowerCase())
      .pop();
  ...
  ngOnInit() {
    ...
    this.myForm = this.formBuilder.group({
      ...
      'department': ['', Validators.compose([Validators.required, Validators.pattern('^[a-zA-Z](?:[a-zA-Z ]*[a-zA-Z])?$'), Validators.maxLength(50)])]
    });
  }
